            ...
            <div class="card shadow-sm border-0">
                ...
                <div class="card-body">
                    <div class="d-flex align-items-center mb-4">
                        <div
                            class="avatar avatar-xl rounded-circle bg-primary-transparent text-primary d-flex align-items-center justify-content-center me-3">
                            <span
                                class="fs-20 fw-bold">{{ strtoupper(substr($employee->first_name, 0, 1)) }}{{ strtoupper(substr($employee->last_name, 0, 1)) }}</span>
                        </div>
                        <div>
                            <h4 class="mb-0">{{ $employee->first_name }} {{ $employee->last_name }}</h4>
                            <p class="text-muted mb-0">
                                {{ $employee->designation->name ?? 'No Designation' }}</p>
                        </div>
                    </div>

                    <div class="row g-3">
                        <div class="col-4">
                            <label class="small text-muted text-uppercase fw-bold">Employee ID</label>
                    ...
                    <div class="card-body">
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <small class="text-muted">Current Balance:
                                                        {{ $employee->getLeaveBalance($type->id) }} days</small>
                                                </td>
                                                <td>
                                                    <input type="number" name="allocations[{{ $type->id }}]" class="form-control"
                        ...
                        @else
                            <div class="alert alert-warning">
                                No active leave types found. Please <a href="{{ route('leaves.types.index') }}">create leave
                                    types</a> first.
                            </div>
                        @endif
                    </div>
